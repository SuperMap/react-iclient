...
describe('iServerRestService', () => {
  ...
  it('should handle getRestMapFields with successful result', (done) => {
    // 模拟 QueryService
    const queryServiceConstructorSpy = jest.spyOn(mapboxgl.supermap, 'QueryService')
      .mockImplementation(function() {
        this.queryBySQL = (param, callback) => {
          callback({
            type: 'processCompleted',
            result: {
              recordsets: [{
                fieldCaptions: ['field1', 'field2']
              }]
            }
          });
        };
    ...
    service._getRestMapFields('https://fakeurl.com', 'layer1', (fields) => {
      expect(fields).toEqual(['field1', 'field2']);
      queryServiceConstructorSpy.mockRestore();
      done();
    });
  });

  it('should handle getRestMapFields with failed result', (done) => {
    // 模拟 QueryService
    const queryServiceConstructorSpy = jest.spyOn(mapboxgl.supermap, 'QueryService')
      .mockImplementation(function() {
        this.queryBySQL = (param, callback) => {
    ...
    service._getRestMapFields('https://fakeurl.com', 'layer1', (result) => {
  ...
  it('should handle getRestMapFields with no result', (done) => {
    // 模拟 QueryService
    const queryServiceConstructorSpy = jest.spyOn(mapboxgl.supermap, 'QueryService')
      .mockImplementation(function() {
        this.queryBySQL = (param, callback) => {
