export const uniqueThemeOptions = {
...
export function initUniqueTheme(callback) {
  ...
  getFeatureBySQLService = new SuperMap.GetFeaturesBySQLService(dataUrl, {
    ...
    eventListeners: { processCompleted: (getFeaturesEventArgs) => processCompleted(getFeaturesEventArgs, callback) }
  });

  getFeatureBySQLService.processAsync(getFeatureBySQLParams);
}

export function processCompleted(getFeaturesEventArgs, callback) {
  ...
  if (result && result.features) {
    ...
    // 岛洞多面要素必需在其他要素之前添加
    const features = IHFeas.concat(feas);
    callback(features);
  }
}

export function islandHoleHandlerForFeature(feature) {
